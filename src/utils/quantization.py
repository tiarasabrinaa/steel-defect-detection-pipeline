...
Kenapa QAT, bukan cuma post-training quantization (PTQ)? README.md
menyebut MobileNetV3 secara eksplisit sebagai "baseline ringan, buat
perbandingan kalau nanti mau deploy edge" (Task A). Defect permukaan baja
di dataset ini (crazing, pitted_surface, rolled-in_scale, dst.) itu soal
...
resnet50, efficientnetv2_s, mobilenetv3. convnext_tiny & swin_tiny TIDAK
didukung - keduanya punya control-flow dinamis (window attention di Swin,
banyak functional reshape/permute) yang bikin `torch.fx.symbolic_trace`
gagal atau butuh custom tracer; di luar scope project ini. Kalau QAT
diminta untuk arsitektur yang tidak didukung, training tetap jalan normal
di fp32 (lihat train_classification.py) - tidak meng-crash run.
...
QAT_SUPPORTED_ARCHITECTURES = ["resnet50", "efficientnetv2_s", "mobilenetv3"]
